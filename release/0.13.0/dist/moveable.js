...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function snapStart(moveable) {
      ...
      elementGuidelines.forEach(function (el) {
        ...
        guidelines.push({
          type: "vertical",
          element: el,
          pos: [round(elementLeft + distLeft), elementTop],
          size: height
        });
        guidelines.push({
          type: "vertical",
          element: el,
          pos: [round(elementRight + distLeft), elementTop],
          size: height
        });
        guidelines.push({
          type: "horizontal",
          element: el,
          pos: [elementLeft, round(elementTop + distTop)],
          size: width
        });
        guidelines.push({
          type: "horizontal",
          element: el,
          pos: [elementLeft, round(elementBottom + distTop)],
          size: width
        });
        ...
        if (snapCenter) {
          guidelines.push({
            type: "vertical",
            element: el,
            pos: [round((elementLeft + elementRight) / 2 + distLeft), elementTop],
            size: height,
            center: true
          });
          guidelines.push({
            type: "horizontal",
            element: el,
            pos: [elementLeft, round((elementTop + elementBottom) / 2 + distTop)],
            size: width,
            center: true
          });
        }
    ...
    function checkSnap(guidelines, targetType, targetPoses, snapThreshold, snapCenter, snapDirection, snapElement) {
      if (!guidelines) {
        return {
          isSnap: false,
          dist: -1,
      ...
      var snapPoses = targetPoses.filter(function (targetPos) {
        return guidelines.filter(function (guideline) {
          ...
          if (!snapElement && element || !snapDirection && !element || !snapCenter && center || type !== targetType) {
            return false;
          }
    ...
    function checkSnapPoses(moveable, posesX, posesY, snapCenter, customSnapThreshold) {
      ...
      var _a = props.snapElement,
          ...
          _b = props.snapHorizontal,
          snapHorizontal = _b === void 0 ? true : _b,
          _c = props.snapVertical,
          snapVertical = _c === void 0 ? true : _c;
      return {
        vertical: checkSnap(guidelines, "vertical", posesX, snapThreshold, snapCenter, snapVertical, snapElement),
        horizontal: checkSnap(guidelines, "horizontal", posesY, snapThreshold, snapCenter, snapHorizontal, snapElement)
      };
    }
